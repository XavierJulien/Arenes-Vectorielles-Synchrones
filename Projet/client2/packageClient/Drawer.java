...
public class Drawer {
	...
	public void drawTargets() {
		for (int i = 0;i< s.getTargets_list().size();i++) {
			Point p = s.getTargets_list().get(i);
			double x = p.getX()+s.getDemil();
			double y = s.getDemih()-p.getY();
			ctx.drawImage(coin, x-objectif_radius,y-objectif_radius, objectif_radius*2, objectif_radius*2);
			if (s.getTargets_list().size()>1)
			...
			ctx.strokeText(String.valueOf(i), x-objectif_radius, y-objectif_radius);
		}
	}
